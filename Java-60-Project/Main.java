...
class Main {
 ...
 public static void main(String[] args)throws IOException{
  ...
  void init() throws IOException{   
    ...
    // Create the database objectc
    Database db = new Database("jdbc:sqlite:fitness.db");
    ...
    String sql = "";
    sql  = " Select * from gym ";
    server.createContext("/gym", new RouteHandler(db,sql) );

    sql  = " Select * from trainer ";
    server.createContext("/trainer", new RouteHandler(db,sql) ) ;

    sql  = " Select * from workout ";
    server.createContext("/workout", new RouteHandler(db,sql) ) ;
